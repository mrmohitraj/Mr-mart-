...
function wireLayoutEvents(){
  ...
  // location pill: simple demo picker
  ...
  if(pill){
    ...
    pill.addEventListener('click', ()=>{
      const val = prompt('Enter your delivery location (e.g. Ludhiana, Punjab):', Storage.get('location','')||'');
      if(val){ Storage.set('location', val); pill.querySelector('span').textContent = val; showToast('Location updated','success'); }
    });
  }
}
  ...
  <div class="product-card fade-in" data-id="${p.id}">
    ...
    <div class="pc-body">
      ...
      <div class="pc-actions">
        <button class="btn btn-outline" data-cart="${p.id}">${t('add')}</button>
        <button class="btn btn-primary" data-buy="${p.id}">${t('buyNow')}</button>
      </div>
    </div>
  </div>`;
}
